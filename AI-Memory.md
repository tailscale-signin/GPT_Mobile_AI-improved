...
> Active Branch: `Ollama` — Prepared exclusively for work on Ollama platform features and improvements.
>
...
> **Current Version:** `0.9.1` (versionCode 33) — LiteRT-LM hardware acceleration, OpenRouter advanced routing, Ollama Advanced Options (`num_gpu`, `num_ctx`, `num_batch`, `num_thread`, `temperature`, `top_p`, `top_k`, `repeat_penalty`, `seed`, `stop`), Room Schema 18 (`MIGRATION_17_18`) adding `ollama_options` to `platform_v2`, dedicated full-screen `McpToolsSelectionScreen` and `AiPlatformsScreen`, reactive flow synchronization between platform cards and platform settings detail toggles, Settings menu feature organization with "AI Platforms" situated cleanly above "Local Models", collapsible Tool Connections with direct inline settings and status badges, bounded layout isolation eliminating thinking/tool-call overlaps, details indicator positioned above user message bubbles, multi-key 3-second round-robin auto-retry on high demand spikes, fancy interactive sorting chips in platform selection dialogs, and comprehensive test suite updates for reactive flows and Room Schema 17/18.
...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingScreen.kt` — Per-platform configuration screen for cloud/local models, parameter tuning, accelerator selection, safety settings, tool bindings, Ollama advanced options dialog item, and separate master/remote/local tool disablement switches.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingDialogs.kt` — Platform settings dialogs including `OllamaAdvancedSettingsDialog` for configuring Ollama execution parameters.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingViewModel.kt` — Platform settings ViewModel managing platform update flows and `isOllamaAdvancedDialogOpen`.
...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/agent/provider/ProviderAdapters.kt` — Adapter streaming logic forwarding `options` in `ChatCompletionRequest` when `compatibleType == ClientType.OLLAMA`.
