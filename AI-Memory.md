...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingScreen.kt` — Reactive Compose screen displaying platform configuration details and dialogs (`PlatformSettingViewModel`, master tool disablement, Ollama advanced dialog trigger, SearchBackendDialog, LegacyMcpToolsDialog).
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingDialogs.kt` — Dialog composables for editing platform properties including `OllamaAdvancedSettingsDialog` and `OpenRouterAdvancedSettingsDialog`.
