...
class AgentToolResolverTest {
    @Test
    fun `zero bindings resolves current date, calculate expression, read file slice, read url, and web search tools`() = runBlocking {
        val resolver = resolver()

        val resolved = resolver.resolve("profile-1")

        assertEquals(listOf("calculate_expression", "current_date", "read_file_slice", "read_url", "web_search"), resolved.map { it.modelToolName })
    ...
    fun `resolve filters bindings to exact profile`() = runBlocking {
        ...
        assertEquals(listOf("calculate_expression", "current_date", "device_location", "read_file_slice", "read_url", "web_search"), resolved.map { it.tool.definition.name })
    ...
    fun `orphan and unknown bindings are ignored while preserving built-in tools`() = runBlocking {
        ...
        assertEquals(listOf("calculate_expression", "current_date", "read_file_slice", "read_url", "web_search"), resolver.resolve("profile-1").map { it.modelToolName })
    }
    ...
    fun `assigned MCP tool is namespaced and executable while unassigned tools stay hidden`() = runBlocking {
        McpClientManagerTest.McpFixtureServer().use { server ->
            ...
            val settingRepository = ResolverFakeSettingRepository()
            val deviceLocationTool = ResolverFakeDeviceLocationTool()
    ...
    fun `one unavailable MCP connection does not hide tools from healthy connections`() = runBlocking {
        McpClientManagerTest.McpFixtureServer().use { server ->
            ...
            assertEquals(listOf("calculate_expression", "current_date", "mcp__mcp-good__echo", "read_file_slice", "read_url", "web_search"), resolved.map { it.modelToolName })
            manager.closeAll()
            networkClient().close()
        }
    }
...
private class ResolverFakeDeviceLocationTool : DeviceLocationTool(
    locationProvider = object : DeviceLocationProvider(android.app.Application()) {
        override fun hasPermission(): Boolean = false
        override suspend fun getCurrentLocation(timeoutMillis: Long): DeviceLocation? = null
    }
)
