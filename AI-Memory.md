...
- **Local inference:** LiteRT-LM (`LocalRuntimeImpl`, conversation fingerprinting, dynamic accelerator selection for NPU/GPU/CPU, warm engine retention across conversational turns, cooperative `yield()` checkpoints, phase-split scheduling `LocalInferencePhase.PREFILL` / `GENERATING`, `RollingContextWindowCompactor` with anchor turn preservation, and tier-aware context window scaling up to 8,192 tokens on >=12GB RAM hardware via `deviceRamGb`); Ollama supported for self-hosted network inference.
