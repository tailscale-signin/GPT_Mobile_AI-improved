...
- Notion remains deferred. Linear, Sentry and Vercel are now configurable presets alongside Atlassian Rovo and Cloudflare Browser Rendering; see [restored features](../missing-features-restoration.md). They do not claim verified Android authorization. Vercel requires an approved client. General browser OAuth support and optional registered client IDs remain available in connection settings.
