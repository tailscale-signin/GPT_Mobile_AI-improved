...
- **Local inference:** LiteRT-LM (`LocalRuntimeImpl`, conversation fingerprinting, dynamic accelerator selection for NPU/GPU/CPU, warm engine retention across conversational turns, cooperative `yield()` checkpoints, phase-split scheduling `LocalInferencePhase.PREFILL` / `GENERATING`, phase-split background progress in `AgentRunForegroundService` displaying "Processing prompt…" and "Generating response…", `RollingContextWindowCompactor` with anchor turn preservation dynamically budgeted against thermal/battery `maxTokensClamp`, `DeviceHardwareGovernor` with adaptive thermal/battery throttling, live `LocalInferenceMetrics` generation telemetry with user notice propagation (`formatTelemetryNotice`), tier-aware context window scaling up to 8,192 tokens on >=12GB RAM hardware via `deviceRamGb`, hardware-aware context ceiling capping via `maxTokensCap()`, and hardware-aware sampling & context default resolution via `localSamplingDefaults(entry, deviceSocModel, deviceRamGb)`); Ollama supported for self-hosted network inference.
