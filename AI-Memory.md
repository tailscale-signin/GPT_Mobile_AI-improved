...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/chat/ThinkingBlock.kt` — Reasoning/thought display block with safe nullable parameter handling and refined 0.25f background alpha.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/telemetry/DiagnosticsTelemetryProvider.kt` — Real-time hardware telemetry provider fetching SoC info, `ActivityManager.MemoryInfo` RAM metrics, power/thermal states, and QNN HTP skeleton presence.

### Qualcomm AI Engine Direct (QNN) & Acceleration

- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/local/QnnEnvironment.kt` — FastRPC dynamic environment configuration, `ADSP_LIBRARY_PATH` configuration, and HTP V79 skeleton library verification.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/local/LocalRuntimeImpl.kt` — Dual LiteRT & QNN execution engine with automatic hardware acceleration targeting (NPU / GPU / CPU).

### Voice, Document RAG & Artifacts
