...
> Active Branch: `Ollama` — Ollama & OpenRouter platform features and timeout resilience: never fails on timeout, continuously retries for up to 5 minutes, supports Android emulator loopback alias fallback (`10.0.2.2`), handles HTTP 502/503/504 gateway & proxy loading codes, and safely wraps up incomplete response if timeout persists.
>
