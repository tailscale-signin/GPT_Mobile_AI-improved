...
> Branch `0.9.0` is dedicated to high-performance hardware acceleration (NPU/GPU speculative decoding, phase-split scheduling, cooperative yielding, persistent in-memory KV-cache pooling, anchor prefix preservation and rolling context truncation, expanded long context up to 8192 tokens for >=12GB/16GB devices, 120Hz frame-synced streaming, real-time inference telemetry, dynamic thermal/battery throttling via DeviceHardwareGovernor, dynamic hardware-aware sampling defaults, phase-split background notification progress in `AgentRunForegroundService`, idle memory auto-unload safeguards in `LocalEngineHolder`, hardware-aware context ceiling resolution & UI guidance, and resilient rate-limiting backoff & credential rotation for OpenAI-compatible and Mistral endpoints).
