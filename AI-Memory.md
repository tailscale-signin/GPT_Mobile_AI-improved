...
> Active Branch: `Ollama` — Ollama & OpenRouter platform features and timeout resilience: never fails on timeout, continuously retries for up to 5 minutes, and safely wraps up incomplete response if timeout persists.
>
...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/agent/provider/ProviderAdapters.kt` — Multi-provider adapter session management (`OpenAIResponsesAdapter`, `OpenAICompatibleAdapter`, `AnthropicMessagesAdapter`, `GeminiAdapter`) with OpenRouter & Ollama parameter injection, and Ollama 5-minute timeout resilience loop.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/repository/SettingRepositoryImpl.kt` — Setting and platform repository implementation managing persistence, SecretVault credential encryption, and Ollama options defaults.
