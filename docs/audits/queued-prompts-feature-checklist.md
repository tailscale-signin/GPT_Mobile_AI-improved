...
- **Swipe feedback and statistics layout:** the conversation ListItem now has a transparent background, allowing the immediate red/cyan Card tint to show through. Statistics range chips wrap on narrow screens and activity labels leave room for their values.
...
| 17 | Immediate red/cyan swipe feedback | `HomeScreen.FancySwipeChatCard` immediately reveals red for delete and cyan for archive, including an early foreground tint and visible action icons. Its inner `ChatListItem` is transparent so its default surface cannot hide that tint. |
