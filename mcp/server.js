...
// 9. get_current_location - Get GPS coordinates from device
server.tool(
  'get_current_location',
  'Get current GPS location from device (requires native bridge)',
  {
    format: z.string().optional().describe('Output format: json, text, address')
  },
  async ({ format = 'json' }) => {
    // Note: This tool requires a native Android/iOS bridge to pass GPS data
    // For MCP server standalone, use get_my_ip() for network-based location instead
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({
          notice: 'GPS access requires native Android/iOS integration. Use get_my_ip() for network-based location detection.',
          status: 'gps_unavailable',
          reason: 'MCP server runs on host, not inside app. Requires companion service or native bridge.'
        }, null, 2)
      }]
    };
  }
...
// 10. geolocate_ip - Look up IP geolocation data
server.tool(
  'geolocate_ip',
  'Look up geolocation data for an IP address (keyless via ipapi.is)',
  {
    ip: z.string().optional().describe('IP address to lookup (auto-detect if omitted)')
  },
  async ({ ip }) => {
...
// 11. reverse_geocode - Convert coordinates to human-readable address
server.tool(
  'reverse_geocode',
