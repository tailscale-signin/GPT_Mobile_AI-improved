...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/chat/ChatBubble.kt` — Message bubbles (`UserChatBubble`, `OpponentChatBubble`) supporting collapsible details, thoughts folding, markdown formatting, run notice chips, and `TelemetryBadge`.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/home/HomeScreen.kt` — Home tab layout (Chats & Favorites) with multi-selection actions, interactive platform sorting chips (`DEFAULT`, `NAME`, `PROVIDER`, `ENABLED_FIRST`), and new chat launcher.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingScreen.kt` — Per-platform configuration screen for cloud/local models, parameter tuning, accelerator selection, safety settings, and tool bindings.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/ToolConnectionsScreen.kt` — Tool connection manager for MCP and Web Search endpoints with multi-key API rotation and OAuth PKCE management.
