...
> Active Branch: `Ollama` — Prepared exclusively for work on Ollama & OpenRouter platform features and improvements.
>
...
> **Current Version:** `0.9.1` (versionCode 33) — LiteRT-LM hardware acceleration, OpenRouter advanced configurable parameters (`stream: true`, `max_tokens: 4096`, `temperature: 0.2`, `top_p: 0.15`, `top_k: 30`, `frequency_penalty: 0.0`, `presence_penalty: 0.0`, `repetition_penalty: 1.03`, `seed: 42`, `provider: { sort: "price-asc", allow_fallbacks: true, skip: ["Mancer"] }`), Ollama Advanced Options (`num_gpu: 999`, `num_ctx: 8192`, `num_batch: 512`, `num_thread: 10`, `temperature: 0.2`, `top_p: 0.15`, `top_k: 30`, `repeat_penalty: 1.1`, `seed: 42`, `stop: ["```end", "delimiter", "You"]`), Room Schema 18 (`MIGRATION_17_18`) adding `ollama_options` to `platform_v2`, dedicated full-screen `McpToolsSelectionScreen` and `AiPlatformsScreen`, reactive flow synchronization between platform cards and platform settings detail toggles, Settings menu feature organization with "AI Platforms" situated cleanly above "Local Models", collapsible Tool Connections with direct inline settings and status badges, bounded layout isolation eliminating thinking/tool-call overlaps, details indicator positioned above user message bubbles, multi-key 3-second round-robin auto-retry on high demand spikes, fancy interactive sorting chips in platform selection dialogs, and comprehensive test suite updates for reactive flows, OpenRouter options, and Room Schema 17/18.
...
- **Local inference:** LiteRT-LM (`LocalRuntimeImpl`, conversation fingerprinting, dynamic accelerator selection for NPU/GPU/CPU, warm engine retention, speculative decoding); Ollama supported for self-hosted network inference with configurable advanced parameters.
...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/chat/ChatScreen.kt` — Core chat UI screen with conversation timeline, streaming progress, model selection, multi-attachment upload, and markdown/LaTeX math rendering.
...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/home/HomeScreen.kt` — Home tab layout (Chats & Favorites) with multi-selection actions, fancy interactive platform sorting chips (`DEFAULT`, `NAME`, `PROVIDER`, `ENABLED_FIRST`), and new chat launcher.
...
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingScreen.kt` — Per-platform configuration screen for cloud/local models, parameter tuning, accelerator selection, safety settings, tool bindings, Ollama & OpenRouter advanced options dialog items ("Advanced"), and separate master/remote/local tool disablement switches.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingDialogs.kt` — Platform settings dialogs including `OllamaAdvancedSettingsDialog` and `OpenRouterAdvancedSettingsDialog` for configuring OpenRouter advanced execution & provider routing parameters (`stream`, `max_tokens`, `temperature`, `top_p`, `top_k`, `frequency_penalty`, `presence_penalty`, `repetition_penalty`, `seed`, `provider`).
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/presentation/ui/setting/PlatformSettingViewModel.kt` — Platform settings ViewModel managing platform update flows, `isOpenRouterSettingsDialogOpen`, and `isOllamaAdvancedDialogOpen`.
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/openrouter/OpenRouterAdvancedOptions.kt` — Data classes and defaults for OpenRouter options (`OpenRouterOptions`, `OpenRouterProviderRouting`).
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/ollama/OllamaOptions.kt` — Data class and defaults for Ollama inference options (`num_gpu`, `num_ctx`, `num_batch`, `num_thread`, `temperature`, `top_p`, `top_k`, `repeat_penalty`, `seed`, `stop`).
- `app/src/main/kotlin/dev/chungjungsoo/gptmobile/data/agent/provider/ProviderAdapters.kt` — Adapter streaming logic forwarding parameters in `ChatCompletionRequest` when `compatibleType == ClientType.OPENROUTER` or `ClientType.OLLAMA`.
