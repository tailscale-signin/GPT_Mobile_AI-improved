...
- **Local inference:** LiteRT-LM (`LocalRuntimeImpl`, conversation fingerprinting, dynamic accelerator selection for NPU/GPU/CPU, warm engine retention across conversational turns, cooperative `yield()` checkpoints, phase-split scheduling `LocalInferencePhase.PREFILL` / `GENERATING`, `RollingContextWindowCompactor` with anchor turn preservation, `DeviceHardwareGovernor` with adaptive thermal/battery throttling, live `LocalInferenceMetrics` generation telemetry, and tier-aware context window scaling up to 8,192 tokens on >=12GB RAM hardware via `deviceRamGb`); Ollama supported for self-hosted network inference.
...
- `agent/` — Autonomous agent orchestration, tool-call loops, limits, background execution, provider adapters, and built-in/MCP tools:
  ...
  - `AgentRunCoordinator.kt` — Singleton coordinator for background/foreground agent execution. Manages concurrent run jobs by `runId`, ensures per-chat serialization via `Mutex` gates, throttles database/UI updates (adaptive 8ms `HIGH_REFRESH_FRAME_INTERVAL_MILLIS` frame budget on >=10GB RAM devices for 120Hz/144Hz displays, 250ms `LOW_POWER_STREAM_PUBLISH_INTERVAL_MILLIS` otherwise), starts `AgentRunForegroundService`, handles cancellation/interruption states, and writes terminal run records to `ChatRepository`.
...
- `localruntime/` — Local inference runtime via LiteRT:
  ...
  - `LocalEngineHolder.kt` — Thread-safe lifecycle holder for the native LiteRT model instance with warm engine retention. Exposes `deviceRamGb`, `getHardwareState()`, and `getAdaptiveThrottlingPolicy()`.
  ...
  - `LocalRuntime.kt` / `LocalRuntimeImpl.kt` — Native on-device execution engine coordinating prompt evaluations, sampling parameters, cooperative yielding (`yield()`), phase-split scheduling (`LocalInferencePhase.PREFILL` and `GENERATING`), live `LocalInferenceMetrics`, hardware thermal/battery adaptation, and streaming token responses. Directly exposes `deviceRamGb: Long`.
...
- `util/` — Utility functions & state flow extensions:
  - `ApiStateFlowExtensions.kt` — Streaming buffer batching and state resolution. Defines `HIGH_REFRESH_FRAME_INTERVAL_MILLIS` (8ms, ~120 FPS frame budget), `STANDARD_STREAM_PUBLISH_INTERVAL_MILLIS` (33ms), and `LOW_POWER_STREAM_PUBLISH_INTERVAL_MILLIS` (250ms).
